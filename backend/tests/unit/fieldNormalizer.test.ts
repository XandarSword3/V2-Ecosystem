...
describe('Field Normalizer Utils', () => {
  describe('snakeToCamel', () => {
    ...
    it('should handle leading underscore', () => {
      // Leading underscore followed by underscore gets converted 
  ...
  describe('normalizeToSnakeCase', () => {
    ...
    it('should handle empty object', () => {
      expect(normalizeToSnakeCase({})).toEqual({});
    });

    it('should handle mixed arrays', () => {
      const input = [{ userName: 'test' }, 'string', 42, null];
      const result = normalizeToSnakeCase(input);
      expect(result).toEqual([{ user_name: 'test' }, 'string', 42, null]);
    });
  });
