...
describe('MaintenanceService', () => {
  ...
  describe('getWorkOrder', () => {
    ...
    it('should reject invalid ID format', async () => {
      await expect(service.getWorkOrder(INVALID_UUID)).rejects.toMatchObject({ code: 'INVALID_WORK_ORDER_ID' });
    });
  });
  ...
  describe('updateWorkOrder', () => {
    ...
    it('should update title', async () => {
      const updated = await service.updateWorkOrder(orderId, { title: 'Replace faucet' });
      expect(updated.title).toBe('Replace faucet');
    });

    it('should update priority', async () => {
      const updated = await service.updateWorkOrder(orderId, { priority: 'high' });
      expect(updated.priority).toBe('high');
    });

    it('should update estimated hours', async () => {
      const updated = await service.updateWorkOrder(orderId, { estimatedHours: 4 });
      expect(updated.estimatedHours).toBe(4);
    });

    it('should reject non-existent order', async () => {
      await expect(service.updateWorkOrder(LOCATION_2, { title: 'New Title' }))
        .rejects.toMatchObject({ code: 'WORK_ORDER_NOT_FOUND' });
    });

    it('should reject invalid ID format', async () => {
      await expect(service.updateWorkOrder(INVALID_UUID, { title: 'New Title' }))
  ...
  describe('deleteWorkOrder', () => {
    ...
    it('should reject non-existent order', async () => {
      await expect(service.deleteWorkOrder(LOCATION_2)).rejects.toMatchObject({ code: 'WORK_ORDER_NOT_FOUND' });
    });
  });
  ...
  describe('assignWorkOrder', () => {
    ...
    it('should reject non-existent order', async () => {
      await expect(service.assignWorkOrder(LOCATION_2, TECHNICIAN_1))
        .rejects.toMatchObject({ code: 'WORK_ORDER_NOT_FOUND' });
    });

    it('should reject invalid assignee ID', async () => {
      await expect(service.assignWorkOrder(orderId, INVALID_UUID))
  ...
  describe('unassignWorkOrder', () => {
    ...
    it('should reject non-assigned order', async () => {
      ...
      await expect(service.unassignWorkOrder(newOrder.id))
  ...
  describe('startWorkOrder', () => {
    ...
    it('should reject unassigned order', async () => {
      ...
      await expect(service.startWorkOrder(newOrder.id))
        .rejects.toMatchObject({ code: 'INVALID_STATUS_TRANSITION' });
    });
  });
  ...
  describe('completeWorkOrder', () => {
    ...
    it('should complete work order', async () => {
      const completed = await service.completeWorkOrder(orderId, { actualHours: 2 });
    ...
    it('should record labor cost', async () => {
      const completed = await service.completeWorkOrder(orderId, { actualHours: 2, laborCost: 150 });
      expect(completed.laborCost).toBe(150);
    });

    it('should calculate parts cost', async () => {
      await service.addPart({ workOrderId: orderId, partName: 'Faucet', quantity: 1, unitCost: 50 });
      await service.addPart({ workOrderId: orderId, partName: 'O-ring', quantity: 2, unitCost: 5 });

      const completed = await service.completeWorkOrder(orderId, { actualHours: 2 });
      expect(completed.partsCost).toBe(60);
    });

    it('should reject negative hours', async () => {
      await expect(service.completeWorkOrder(orderId, { actualHours: -1 }))
        .rejects.toMatchObject({ code: 'INVALID_HOURS' });
    });
    ...
    it('should reject non-started order', async () => {
      ...
      await expect(service.completeWorkOrder(newOrder.id, { actualHours: 1 }))
        .rejects.toMatchObject({ code: 'INVALID_STATUS_TRANSITION' });
    });
  });
  ...
  describe('cancelWorkOrder', () => {
    ...
    it('should reject already cancelled order', async () => {
      ...
      await expect(service.cancelWorkOrder(orderId)).rejects.toMatchObject({ code: 'ALREADY_CANCELLED' });
    });
  });
  ...
  describe('reopenWorkOrder', () => {
    ...
    it('should reject non-cancelled order', async () => {
      ...
      await expect(service.reopenWorkOrder(newOrder.id))
        .rejects.toMatchObject({ code: 'INVALID_STATUS_TRANSITION' });
    });
  });
  ...
  describe('addPart', () => {
    ...
    it('should add part to work order', async () => {
      const part = await service.addPart({ workOrderId: orderId, partName: 'Faucet', quantity: 1, unitCost: 50 });
    ...
    it('should calculate total cost', async () => {
      const part = await service.addPart({ workOrderId: orderId, partName: 'O-ring', quantity: 5, unitCost: 2 });
      expect(part.totalCost).toBe(10);
    });

    it('should accept part number', async () => {
      const part = await service.addPart({ workOrderId: orderId, partName: 'Faucet', partNumber: 'FT-1234', quantity: 1, unitCost: 50 });
      expect(part.partNumber).toBe('FT-1234');
    });

    it('should reject empty part name', async () => {
      await expect(service.addPart({ workOrderId: orderId, partName: '', quantity: 1, unitCost: 10 }))
        .rejects.toMatchObject({ code: 'INVALID_PART_NAME' });
    });

    it('should reject invalid quantity', async () => {
      await expect(service.addPart({ workOrderId: orderId, partName: 'Part', quantity: 0, unitCost: 10 }))
        .rejects.toMatchObject({ code: 'INVALID_QUANTITY' });
    });

    it('should reject negative unit cost', async () => {
      await expect(service.addPart({ workOrderId: orderId, partName: 'Part', quantity: 1, unitCost: -10 }))
  ...
  describe('getParts', () => {
    it('should return parts for work order', async () => {
      ...
      await service.addPart({ workOrderId: order.id, partName: 'Part 1', quantity: 1, unitCost: 10 });
      await service.addPart({ workOrderId: order.id, partName: 'Part 2', quantity: 2, unitCost: 20 });
    ...
    it('should reject non-existent order', async () => {
      await expect(service.getParts(LOCATION_2)).rejects.toMatchObject({ code: 'WORK_ORDER_NOT_FOUND' });
    });
  });

  describe('listWorkOrders', () => {
    beforeEach(async () => {
      await service.createWorkOrder({ title: 'Fix faucet', description: 'Plumbing issue', category: 'plumbing', priority: 'medium', locationId: LOCATION_1, locationType: 'room', reportedBy: REPORTER_1 });
      await service.createWorkOrder({ title: 'Fix AC', description: 'HVAC issue', category: 'hvac', priority: 'high', locationId: LOCATION_1, locationType: 'room', reportedBy: REPORTER_1 });
      await service.createWorkOrder({ title: 'Fix lights', description: 'Electrical issue', category: 'electrical', priority: 'critical', locationId: LOCATION_2, locationType: 'lobby', reportedBy: REPORTER_1 });
    });
  ...
  describe('getByLocation', () => {
    it('should return work orders by location', async () => {
      await service.createWorkOrder({ title: 'Fix faucet', description: 'Description', category: 'plumbing', priority: 'medium', locationId: LOCATION_1, locationType: 'room', reportedBy: REPORTER_1 });
  ...
  describe('getOpenWorkOrders', () => {
    it('should return only open/active work orders', async () => {
      const order1 = await service.createWorkOrder({ title: 'Open order', description: 'Description', category: 'general', priority: 'low', locationId: LOCATION_1, locationType: 'room', reportedBy: REPORTER_1 });
      const order2 = await service.createWorkOrder({ title: 'Cancelled order', description: 'Description', category: 'general', priority: 'low', locationId: LOCATION_1, locationType: 'room', reportedBy: REPORTER_1 });
  ...
  describe('getCriticalWorkOrders', () => {
    it('should return only critical open work orders', async () => {
      await service.createWorkOrder({ title: 'Critical order', description: 'Description', category: 'electrical', priority: 'critical', locationId: LOCATION_1, locationType: 'room', reportedBy: REPORTER_1 });
      await service.createWorkOrder({ title: 'Low order', description: 'Description', category: 'general', priority: 'low', locationId: LOCATION_1, locationType: 'room', reportedBy: REPORTER_1 });
  ...
  describe('getStats', () => {
    ...
    it('should count work orders by status', async () => {
      await service.createWorkOrder({ title: 'Order 1', description: 'Description', category: 'plumbing', priority: 'medium', locationId: LOCATION_1, locationType: 'room', reportedBy: REPORTER_1 });
      const stats = await service.getStats();
