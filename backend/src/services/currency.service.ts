...
class CurrencyService {
  ...
  async getDefaultCurrency(): Promise<Currency> {
    ...
    if (!currency) throw new Error(`Default currency ${defaultCode} not found`);
    return currency;
  }

  async convert(amount: number, fromCurrency: string, toCurrency: string): Promise<PriceConversion> {
    fromCurrency = fromCurrency.toUpperCase();
    toCurrency = toCurrency.toUpperCase();
    if (fromCurrency === toCurrency) {
      return { original_amount: amount, original_currency: fromCurrency, converted_amount: amount, target_currency: toCurrency, exchange_rate: 1, converted_at: new Date().toISOString() };
    }
    ...
    return { original_amount: amount, original_currency: fromCurrency, converted_amount: roundedAmount, target_currency: toCurrency, exchange_rate: exchangeRate, converted_at: new Date().toISOString() };
  }
  ...
  async updateExchangeRate(currencyCode: string, rate: number, adminUserId?: string): Promise<Currency> {
    currencyCode = currencyCode.toUpperCase();
    const { data, error } = await supabase
      .from('currencies')
      .update({ exchange_rate: rate, last_updated: new Date().toISOString() })
  ...
  async updateAllRatesFromAPI(): Promise<void> {
    ...
    if (!API_KEY) { console.warn('[CurrencyService] No exchange rate API key configured'); return; }
    try {
      const response = await fetch(`https://api.exchangerate-api.com/v4/latest/${this.defaultCurrency}`, { headers: { 'Authorization': `Bearer ${API_KEY}` } });
      if (!response.ok) throw new Error(`API error: ${response.status}`);
      const data = await response.json() as any;
      const rates = data.rates as Record<string, number>;
      for (const code of Object.keys(SUPPORTED_CURRENCIES)) {
        if (rates[code]) await this.updateExchangeRate(code, rates[code]);
      }
  ...
  formatAmount(amount: number, currencyCode: string, locale?: string): string {
    return new Intl.NumberFormat(locale || 'en-US', { style: 'currency', currency: currencyCode.toUpperCase() }).format(amount);
  }

  getStripeCurrency(currencyCode: string): string { return currencyCode.toLowerCase(); }

  toStripeAmount(amount: number, currencyCode: string): number {
    const currency = SUPPORTED_CURRENCIES[currencyCode.toUpperCase()];
    return Math.round(amount * Math.pow(10, currency?.decimal_places ?? 2));
  }
  ...
  async getMultiCurrencyPrices(baseAmount: number, baseCurrency: string, targetCurrencies?: string[]): Promise<Record<string, { amount: number; formatted: string }>> {
    ...
    for (const currency of currencies) {
      ...
      prices[currency] = { amount: conversion.converted_amount, formatted: this.formatAmount(conversion.converted_amount, currency) };
    }
    return prices;
  }

  async initializeCurrencies(): Promise<void> {
    const defaultRates: Record<string, number> = { EUR: 1, USD: 1.08, GBP: 0.86, AED: 3.97, SAR: 4.05, QAR: 3.93, KWD: 0.33, CHF: 0.95 };
    for (const [code, info] of Object.entries(SUPPORTED_CURRENCIES)) {
      const { data: existing } = await supabase.from('currencies').select('code').eq('code', code).single();
      if (!existing) {
        await supabase.from('currencies').insert({ ...info, exchange_rate: defaultRates[code] || 1, is_active: true, is_default: code === 'EUR', last_updated: new Date().toISOString() });
      }
    }
