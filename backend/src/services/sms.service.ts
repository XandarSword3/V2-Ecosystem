...
const SMS_TEMPLATES: Record<string, (data: Record<string, any>) => string> = {
  'booking-confirmation': (data) =>
    `V2 Ecosystem: Your booking #${data.booking_id} is confirmed for ${data.metadata?.check_in_date}. Check-in: ${data.metadata?.check_in_time}. Questions? Call ${data.resort_phone}`,

  'booking-reminder': (data) =>
    `V2 Ecosystem: Reminder - Your stay begins ${data.metadata?.check_in_date}. Looking forward to hosting you! Check-in from ${data.metadata?.check_in_time}.`,
