...
const SMS_TEMPLATES: Record<string, (data: Record<string, any>) => string> = {
  'booking-confirmation': (data) =>
    `V2 Ecosystem: Your booking #${data.booking_id} is confirmed for ${(\1.metadata as any)?.check_in_date_date}. Check-in: ${(\1.metadata as any)?.check_in_date_time}. Questions? Call ${data.resort_phone}`,

  'booking-reminder': (data) =>
    `V2 Ecosystem: Reminder - Your stay begins ${(\1.metadata as any)?.check_in_date_date}. Looking forward to hosting you! Check-in from ${(\1.metadata as any)?.check_in_date_time}.`,
