...
/**
 * Single-writer atomic lease claim with ownership token for commercial checkout requests (Phase F4).
 * Enforces single executor, eliminates concurrent race executions, and supports safe retries on failure.
 */
async function claimIdempotency(supabase: any, key: string, leaseSeconds: number = 60): Promise<IdempotencyClaimResult> {
  const now = new Date();
  const expiresAt = new Date(now.getTime() + leaseSeconds * 1000).toISOString();
  const token = crypto.randomUUID();

  try {
    const { data: rpcResult, error: rpcError } = await supabase.rpc('claim_idempotency_key', {
      p_key: key,
      p_lease_seconds: leaseSeconds,
    });
    if (!rpcError && rpcResult && typeof rpcResult === 'object') {
      return rpcResult as IdempotencyClaimResult;
    }
  } catch (_) {}

  // Direct database claim fallback (atomic insert with token)
  const { data: inserted, error: insertErr } = await supabase
    ...
    .select('key, claim_token, status, response, transaction_id, expires_at')
    .maybeSingle();

  if (!insertErr && inserted) {
    return { claimed: true, status: 'in_progress', claim_token: token };
  }
...
/**
 * Assert that the active lease is still valid, held by this worker, and not expired (Phase F4).
...
async function assertActiveLease(supabase: any, key: string | null, claimToken: string | null): Promise<boolean> {
  ...
  const { data } = await supabase
    .from('idempotency_records')
    .select('claim_token, status, expires_at')
    .eq('key', key)
    .maybeSingle();

  const isNotExpired = data?.expires_at ? new Date(data.expires_at).getTime() > Date.now() : true;
  return Boolean(data && data.claim_token === claimToken && data.status === 'in_progress' && isNotExpired);
}
 ...
 * and records failed steps to checkout_compensation_queue for asynchronous recovery.
 */
async function rollbackFailedOrder(
  supabase: any,
  transactionId: string | null,
  baseInventoryItems: any[],
  ...
  if (baseInventoryItems && baseInventoryItems.length > 0) {
    try {
      ...
      try {
        await supabase.rpc('queue_failed_compensation', {
          p_key: scopedIdempotencyKey,
          p_transaction_id: transactionId,
          p_operation: 'restore_inventory',
          p_payload: baseInventoryItems,
          p_error: invErr?.message || String(invErr),
        });
      } catch (_) {}
    }
  }
    ...
    try {
      ...
      try {
        await supabase.rpc('queue_failed_compensation', {
          p_key: scopedIdempotencyKey,
          p_transaction_id: transactionId,
          p_operation: 'reverse_discounts',
          p_payload: discounts,
          p_error: discErr?.message || String(discErr),
        });
      } catch (_) {}
    }
  }
    ...
    try {
      ...
      try {
        await supabase.rpc('queue_failed_compensation', {
          p_key: scopedIdempotencyKey,
          p_transaction_id: transactionId,
          p_operation: 'delete_order_items',
          p_payload: { transaction_id: transactionId },
          p_error: oiErr?.message || String(oiErr),
        });
      } catch (_) {}
    }
    ...
    try {
      ...
      try {
        await supabase.rpc('queue_failed_compensation', {
          p_key: scopedIdempotencyKey,
          p_transaction_id: transactionId,
          p_operation: 'delete_transaction',
          p_payload: { transaction_id: transactionId },
          p_error: txErr?.message || String(txErr),
        });
      } catch (_) {}
    }
  }
      ...
      try {
        ...
        try {
          await supabase.rpc('queue_failed_compensation', {
            p_key: scopedIdempotencyKey,
            p_transaction_id: transactionId,
            p_operation: 'fail_idempotency_key',
            p_payload: { key: scopedIdempotencyKey, claim_token: claimToken },
            p_error: updateErr?.message || String(updateErr),
          });
        } catch (_) {}
      }
    }
...
function buildInstantTransactionRouter(router: Router): void {
  ...
  router.post('/orders', authorize('customer', ...STAFF_ROLES), async (req: Request, res: Response) => {
    ...
    let baseInventoryDeducted = false;
    ...
    try {
      ...
      scopedIdempotencyKey = rawIdempotencyKey && typeof rawIdempotencyKey === 'string'
        ? `${tenantId}:${propertyId}:${mounted.id}:${customerScope}:${rawIdempotencyKey}`
        : null;

      if (scopedIdempotencyKey) {
        let claimResult = await claimIdempotency(supabase, scopedIdempotencyKey);

        if (!claimResult.claimed) {
          if (claimResult.status === 'completed' && claimResult.response) {
            logger.info('[Dynamic Router] Idempotent checkout hit (completed): returning cached response', {
              key: scopedIdempotencyKey,
            });
            return res.json(claimResult.response);
          }

          if (claimResult.status === 'in_progress') {
            // Concurrent request in flight: poll for completion
            ...
            for (let attempt = 0; attempt < 25; attempt++) {
              ...
              if (polled && polled.status === 'completed' && polled.response) {
                resolvedRecord = polled;
                break;
              }
              if (polled && polled.status === 'failed') {
                const retryClaim = await claimIdempotency(supabase, scopedIdempotencyKey);
                if (retryClaim.claimed) {
                  claimResult = retryClaim;
            ...
            if (resolvedRecord) {
              return res.json(resolvedRecord.response);
            }
            ...
            if (!claimResult.claimed) {
              return res.status(409).json({
                success: false,
                error: 'IN_PROGRESS',
                message: 'Your order is currently being processed. Please check back in a moment.',
              });
            }
          }
        } else {
      ...
      // Phase F4: Atomic transaction creation under lease ownership boundary
      ...
      try {
        ...
        logger.error('[Dynamic Router] Exception during discount linking, rolling back order:', linkErr);
      ...
      try {
        ...
        try {
          const { data: atomicItemsResult, error: rpcItemsErr } = await supabase.rpc('persist_order_items_atomic', {
            p_key: scopedIdempotencyKey,
            p_claim_token: claimToken,
        ...
        if (orderItemsError) {
          logger.error('[Dynamic Router] Failed to persist order_items, rolling back:', {
            orderId: created.id,
            error: orderItemsError.message || String(orderItemsError),
          });
          await rollbackFailedOrder(
            supabase,
            created.id,
      ...
      // Persist to financial ledger (non-fatal)
      try {
      ...
      if (scopedIdempotencyKey && claimToken) {
        try {
          const { data: rpcCompleted } = await supabase.rpc('complete_idempotency_key', {
            p_key: scopedIdempotencyKey,
            p_claim_token: claimToken,
            p_transaction_id: created.id,
