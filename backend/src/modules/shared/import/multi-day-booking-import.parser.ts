/**
 * Multi-Day Booking Import Parser
 * Engine: multi_day_booking
 * 
 * Handles parsing of accommodation/bookable units from JSON or LLM text input.
 * Used by all multi_day_booking type modules (chalets, hotel rooms, vacation rentals, etc.)
 */
...
export async function parseLlmImport(text: string): Promise<UnitImportResult> {
  ...
  try {
    const rawItems = await callLlmParser(LLM_SYSTEM_PROMPTS.multi_day_booking, text);
