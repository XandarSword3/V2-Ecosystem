/**
 * Session Access Import Parser
 * Engine: session_access
 * 
 * Handles parsing of session configurations from JSON or LLM text input.
 * Used by all session_access type modules (pool, spa, gym, etc.)
 */
...
export async function parseLlmImport(text: string): Promise<SessionImportResult> {
  ...
  try {
    const rawItems = await callLlmParser(LLM_SYSTEM_PROMPTS.session_access, text);
