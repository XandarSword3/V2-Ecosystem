...
`,
  ...
  paths: {
    ...
    '/staff/modules/{moduleSlug}/orders/{id}/status': { put: { tags: ['Dynamic Modules'], operationId: 'updateModuleOrderStatus', summary: 'Update order status (staff)', security: secured, parameters: [{ name: 'moduleSlug', in: 'path', required: true, schema: { type: 'string' } }, idParam('Order')], responses: { '200': jsonResponse('Updated'), '404': responses.NotFound } }, patch: { tags: ['Dynamic Modules'], operationId: 'updateModuleOrderStatusPatch', summary: 'Update order status (staff)', security: secured, parameters: [{ name: 'moduleSlug', in: 'path', required: true, schema: { type: 'string' } }, idParam('Order')], responses: { '200': jsonResponse('Updated'), '404': responses.NotFound } } },
