# Walkthrough Documentation

> **Total Walkthroughs:** 11 | **Engine Types:** 4 | **Modules:** 37

## Scope
This README documents the walkthrough directory within the V2 Resort platform's 37-module architecture.

## Verification
- Last refreshed: 2026-06-24
- Verification source: confirmed ground truth from codebase analysis
- Runtime and browser-level validation is consolidated in the main project README.

## Walkthrough Overview

The walkthrough documentation provides step-by-step guides for:

- **4-engine transaction framework** implementation examples
- **Module creation** and E2E testing procedures
- **Admin operations** across all engine types
- **Customer journey** testing and validation
- **Staff operations** and workflow verification
- **Property creation and multi-tenant isolation** mechanics and configuration

## Directory Inventory
### Subdirectories (0)
- None

### Files (10)
- 01-bug-fix-hydration-error.md
- 02-cms-rebranding.md
- 03-module-creation.md
- 04-admin-module-population.md
- 05-customer-journeys.md
- 06-staff-operations.md
- 07-admin-dashboard-verification.md
- 08-offline-mode-testing.md
- 09-new-module-e2e-testing.md
- 10-property-creation-and-isolation.md

## Local Commands
- No package scripts found in this directory.

## Maintenance Notes
- Keep this README aligned with the real files and scripts in this directory.
- Add deeper implementation notes when new modules, routes, or services are introduced.
