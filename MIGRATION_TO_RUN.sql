-- =============================================
-- Menu Modifiers Complete System - Run in Supabase SQL Editor
-- https://supabase.com/dashboard/project/dfneswicpdprhneeqlsn/sql
-- =============================================

-- 1. Add modifier type and inventory linking to menu_modifier_options
...
-- 2. Add constraint for modifier_type
DO $$
BEGIN
...
-- 4. Add modifier columns to restaurant_order_items
...
-- 5. Add modifiers_total to restaurant_orders
ALTER TABLE restaurant_orders 
  ADD COLUMN IF NOT EXISTS modifiers_total DECIMAL(10,2) DEFAULT 0;

-- 6. Add modifier columns to snack_bar_orders if exists
DO $$
BEGIN
...
-- 7. Create snack_bar_item_modifiers table if not exists
...
WHERE table_name = 'menu_modifier_options' 
  AND column_name IN ('modifier_type', 'inventory_item_id', 'quantity_required', 'unit', 'description', 'max_quantity', 'is_default')
