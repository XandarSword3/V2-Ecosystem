...
export default function DynamicSessionsPage() {
  ...
  const fetchSessions = async () => {
    ...
    try {
      setLoading(true);
      const response = await api.get(`/${currentModule.slug}/sessions`, { params: { moduleId: currentModule.id } });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingSession) {
        await api.put(`/${currentModule.slug}/admin/sessions/${editingSession.id}`, payload);
        toast.success('Session updated successfully');
      } else {
        await api.post(`/${currentModule.slug}/admin/sessions`, payload);
  ...
  const handleDelete = async (id: string) => {
    ...
    if (!currentModule) return;
    try {
      await api.delete(`/${currentModule.slug}/admin/sessions/${id}`);
  ...
  const toggleActive = async (session: Session) => {
    if (!currentModule) return;
    try {
      await api.put(`/${currentModule.slug}/admin/sessions/${session.id}`, { is_active: !session.is_active });
      fetchSessions();
