...
export default function DynamicCapacityPage() {
  ...
  const fetchCapacity = useCallback(async () => {
    ...
    try {
      const response = await api.get(`/${currentModule.slug}/staff/capacity`, { params: { moduleId: currentModule.id } });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await api.put(`/${currentModule.slug}/admin/settings`, { maxCapacity: capacity.max_capacity });
  ...
  const resetCurrent = async () => {
    if (!currentModule) return;
    try {
      await api.post(`/${currentModule.slug}/admin/reset-occupancy`);
