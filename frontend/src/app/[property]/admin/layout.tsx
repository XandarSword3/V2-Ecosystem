...
export default function AdminLayout({ children }: AdminLayoutProps) {
  ...
  const navigation = useMemo((): NavCategory[] => {
    const categories = getStaticNavigation(t, propertySlug);
    ...
    if (modulesCategory && modules && modules.length > 0) {
      ...
      modulesCategory.items = activeModules.map(module => {
        ...
        const children = getModuleChildren(module.slug, module.engine_type, t, propertySlug);
        
        return {
          name: module.name,
          href: `/${propertySlug}/admin/${encodeURIComponent(module.slug)}`,
          icon,
  ...
  useEffect(() => {
    if (!isLoading) {
      setAuthChecked(true);
      if (!isAuthenticated) {
        router.push(`/login?redirect=/${propertySlug}/admin`);
    ...
    async function checkOnboarding() {
      try {
        ...
        if (res.data?.success && res.data?.data) {
          ...
          const adminSetupPath = `/${propertySlug}/admin/setup`;
          if (!completed && pathname !== adminSetupPath) {
            router.push(adminSetupPath);
          }
        } else {
          setOnboardingChecked(true);
        }
  ...
  if (isLoading || !authChecked || !isAuthenticated || (!onboardingChecked && !onboardingBypassEnabled && pathname !== `/${propertySlug}/admin/setup`)) {
    return (
            ...
            <div className="flex items-center gap-3 px-3 py-2 mb-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 truncate">
                  {user?.email || 'admin@yourresort.com'}
                </p>
              </div>
            </div>
          )}
              ...
              <AnimatePresence>
                {notificationsOpen && (
                  ...
                  >
                    ...
                    <Link
                      href={`/${propertySlug}/admin/settings/notifications`}
