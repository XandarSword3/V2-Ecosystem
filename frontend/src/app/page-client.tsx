...
export default function HomePage() {
  ...
  const heroSlides = useMemo(() => {
    const customSlides = settings.homepage?.heroSlides?.filter(s => s.enabled) || [];
    const defaultSlide = {
      id: 'default',
      title: settings.siteName || tHome('hero.titleHighlight'),
    ...
    // If includeDefaultSlide is true, prepend the default slide to custom slides
    // Otherwise, use custom slides only, or fall back to default if no custom slides exist
    if (settings.homepage?.includeDefaultSlide && customSlides.length > 0) {
      return [defaultSlide, ...customSlides];
    }
    return customSlides.length > 0 ? customSlides : [defaultSlide];
  }, [settings.homepage?.heroSlides, settings.homepage?.includeDefaultSlide, settings.siteName, settings.description, tHome]);

  // Auto-rotate hero slides
  useEffect(() => {
    if (heroSlides.length <= 1) return;
    const intervalSeconds = typeof settings.homepage?.rotationInterval === 'number'
      ? settings.homepage.rotationInterval
      : 6; // default 6 seconds
    
    if (intervalSeconds === 0) return; // manual only

    const interval = setInterval(() => {
      setCurrentSlideIndex(prev => (prev + 1) % heroSlides.length);
    }, intervalSeconds * 1000);
    return () => clearInterval(interval);
  }, [heroSlides.length, settings.homepage?.rotationInterval]);
  ...
  const ctaSettings = useMemo(() => ({
    title: settings.homepage?.ctaTitle || `Ready to Experience ${settings.siteName || 'Our Services'}?`,
    ...
    buttonLink: settings.homepage?.ctaButtonLink || '/',
  }), [settings.homepage, settings.siteName]);
  ...
  const stats = useMemo(() => {
    ...
    return activeModules.slice(0, 4).map(module => {
      ...
      return {
        ...
        label: module.name,
        suffix: module.engine_type === 'instant_transaction' ? '+' : '',
    ...
    <AuroraBackground className="min-h-screen" intensity="medium">
      ...
      <section ref={heroRef} className="relative min-h-[100vh] flex items-center justify-center pt-20 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
                ...
                {currentSlide.imageUrl ? (
                  <span className="text-4xl sm:text-6xl md:text-8xl lg:text-9xl font-black text-white drop-shadow-xl">
                    <StaggerText delay={0.4} staggerDelay={0.08}>
                      {settings.homepage?.hero?.title || currentSlide.title || settings.siteName || 'Welcome'}
                    </StaggerText>
                  </span>
                ) : (
          ...
          >
            <Link href={settings.homepage?.hero?.ctaLink || currentSlide.buttonLink || '/'}>
                ...
                <h2 className="text-4xl md:text-5xl font-bold text-slate-900 dark:text-white mb-8">
                  ...
                  <span className="relative inline-block">
                    <span className="text-primary-600 dark:text-primary-400">
                      {' '}{settings.siteName || tHome('hero.titleHighlight')}
                    </span>
                    <motion.span
      ...
      {/* Property Map Section - Conditionally rendered */}
          ...
          <div className="max-w-5xl mx-auto">
            ...
            >
              ...
              <RevealHeading className="text-3xl md:text-4xl font-bold text-slate-900 dark:text-white">
                {tCommon('exploreResort') || 'Explore Our Property'}
              </RevealHeading>
            </motion.div>
            <Card3D intensity={5}>
