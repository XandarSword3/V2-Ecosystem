  ...
  const openGroupModal = (group?: ModifierGroup) => {
    if (group) {
      ...
      setGroupForm({
        ...
        allowMultipleSame: group.allow_multiple_same,
      });
    } else {
  ...
  const saveGroup = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        allowMultipleSame: groupForm.allowMultipleSame,
              ...
              <div className="p-4 sm:p-6 space-y-4">
                ...
                <div className="flex flex-wrap gap-4">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={groupForm.allowMultipleSame}
                      onChange={(e) => setGroupForm({ ...groupForm, allowMultipleSame: e.target.checked })}
                      className="w-4 h-4 rounded border-slate-300 text-primary"
                    />
                    <span className="text-sm text-slate-700 dark:text-slate-300">Allow multiple of same option</span>
                  </label>
                </div>
              </div>
