...
/**
 * DestinationSelector — Canonical Component for Customer Destination Input.
 *
 * Dynamically mounts mode-specific destination inputs derived from the active
 * canonical fulfillment mode (Phase F7).
 */
export function DestinationSelector({
  ...
  if (mode === 'none') {
    return (
      <div
        className={`p-4 rounded-xl bg-muted/40 border border-border/40 text-sm text-muted-foreground flex items-center gap-3 ${className}`}
        data-testid="destination-selector-none"
      >
  ...
  if (mode === 'on_premise') {
    ...
    return (
      <div className={`space-y-3 ${className}`} data-testid="destination-selector-on-premise">
        ...
        {loadingLocations ? (
          <div className="py-4 text-xs text-muted-foreground animate-pulse" data-testid="destination-loading-locations">
  ...
  if (mode === 'pickup') {
    return (
      <div className={`space-y-3 ${className}`} data-testid="destination-selector-pickup">
        <label className="block text-sm font-semibold text-foreground">
        ...
        <input
          ...
          className="w-full px-4 py-2.5 rounded-lg border border-border bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 disabled:opacity-50"
        />
      </div>
    );
  }
  ...
  if (mode === 'local_delivery' || mode === 'shipment') {
    return (
      <div className={`space-y-3 ${className}`} data-testid="destination-selector-delivery">
        <label className="block text-sm font-semibold text-foreground">
        ...
        <textarea
          ...
          className="w-full px-4 py-2.5 rounded-lg border border-border bg-background text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 resize-none disabled:opacity-50"
        />
      </div>
    );
  }
  ...
  if (mode === 'digital_delivery') {
    return (
      <div className={`space-y-3 ${className}`} data-testid="destination-selector-digital">
        <label className="block text-sm font-semibold text-foreground">
  ...
  if (mode === 'service_execution') {
    return (
      <div className={`space-y-3 ${className}`} data-testid="destination-selector-service">
        <label className="block text-sm font-semibold text-foreground">
...
export const DestinationRequirementsEditor = DestinationSelector;
export type DestinationRequirementsEditorProps = DestinationSelectorProps;
