...
describe('auth context', () => {
  ...
  it('returns two-factor challenge when backend requires 2FA', async () => {
    ...
    let result: unknown;
    await act(async () => {
      result = await hook.result.current.login('2fa@resort.test', 'secret');
    });
  ...
  it('stores tokens and user on successful login and verify2FA', async () => {
    ...
    await act(async () => {
      await hook.result.current.login('login@resort.test', 'secret');
    });
