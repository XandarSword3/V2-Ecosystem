 ...
 */
export type EngineType = 'instant_transaction' | 'time_exclusive_reservation' | 'shared_capacity_access' | 'ongoing_entitlement';
/** Database template_type → engine type mapping */
export declare const TEMPLATE_TO_ENGINE: Record<string, EngineType>;
export declare const ENGINE_TO_TEMPLATE: Record<EngineType, string>;
/**
...
export type InstantTransactionStatus = 'pending' | 'confirmed' | 'preparing' | 'ready' | 'delivered' | 'completed' | 'cancelled';
export type TimeExclusiveReservationStatus = 'pending' | 'confirmed' | 'checked_in' | 'checked_out' | 'cancelled' | 'no_show';
export type SharedCapacityAccessStatus = 'valid' | 'active' | 'used' | 'expired' | 'cancelled';
export type OngoingEntitlementStatus = 'pending' | 'active' | 'paused' | 'expired' | 'cancelled';
