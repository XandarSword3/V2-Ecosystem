-- reserve_unit_exclusive_atomic's INSERT into transactions never set tenant_id or
-- property_id. Both are NOT NULL with no column default and no BEFORE INSERT trigger
-- populates them, so every real call to this function raises:
--   null value in column "property_id" of relation "transactions" violates not-null constraint
-- (tenant_id would fail identically once property_id is fixed).
--
-- modules.tenant_id and modules.property_id are NOT NULL and always populated, so derive
-- both from the module the reservation belongs to, matching the pattern already used in
-- purchase_shared_capacity_atomic (which takes property_id as a param sourced from the
-- caller's session/module context).
