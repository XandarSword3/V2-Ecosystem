-- Fix: discount_amount/tax_amount were never written on time_exclusive_reservation
-- bookings (always defaulted to 0), which silently breaks the economics RPCs that
-- compute gross = amount + discount_amount for reporting.
-- Adds two optional params to the atomic reservation function and writes them in
-- the same INSERT as the reservation itself (atomic — no follow-up unchecked update).
