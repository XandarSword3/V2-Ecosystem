-- Create a function to insert pool sessions, bypassing PostgREST schema cache issues
...
DECLARE
  result JSONB;
BEGIN
  INSERT INTO pool_sessions (name, start_time, end_time, max_capacity, adult_price, child_price, gender_restriction, module_id)
  VALUES (p_name, p_start_time, p_end_time, p_max_capacity, p_adult_price, p_child_price, p_gender_restriction, p_module_id)
  RETURNING to_jsonb(pool_sessions.*) INTO result;
  RETURN result;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Also create update and delete functions for consistency
