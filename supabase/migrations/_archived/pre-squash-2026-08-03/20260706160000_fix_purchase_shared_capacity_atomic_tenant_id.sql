-- Same pattern bug as reserve_unit_exclusive_atomic: transactions.tenant_id/property_id
-- are NOT NULL with no default. This function never set tenant_id at all (would fail on
-- every real call), and only sets property_id from a caller-supplied param with no
-- server-side fallback (fails if the caller ever omits it, e.g. propertyId ?? null in
-- shared-capacity-purchase.ts).
--
-- Fix: derive both tenant_id and property_id from the module (same module_id already
-- resolved via COALESCE(p_module_id, v_session.module_id) below), falling back to the
-- caller-supplied p_property_id only if explicitly provided and non-null.
