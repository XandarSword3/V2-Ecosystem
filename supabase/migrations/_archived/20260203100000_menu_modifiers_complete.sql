...
-- 4. Add selected_modifiers and modifier_total to order_items (if table exists)
...
-- 8. modifiers_total tracked in transactions.metadata
-- (no separate column needed on transactions; use metadata->>'modifiers_total')
DO $$ BEGIN NULL; END $$; -- restaurant_orders removed
...
BEGIN
    ...
    FOR v_group IN 
        ...
        WHERE mim.catalog_item_id = p_catalog_item_id
...
CREATE OR REPLACE FUNCTION deduct_modifier_inventory(
    p_order_id UUID,
    p_order_items JSONB -- Array of {catalog_item_id, quantity, selected_modifiers}
)
RETURNS TABLE (
    items_deducted INT,
...
-- 12. deduct_inventory_for_order_v2: reads from order_items (linked to transactions)
...
BEGIN
    FOR v_order_item IN
        ...
        FROM order_items oi
        JOIN menu_items mi ON oi.catalog_item_id = mi.id
    ...
    LOOP
        ...
        FOR v_ingredient IN
            ...
            WHERE mii.catalog_item_id = v_order_item.catalog_item_id
        LOOP
        ...
        LOOP
            ...
            IF FOUND THEN
                INSERT INTO inventory_transactions (
                    ...
                    'transaction_modifier', p_transaction_id,
...
CREATE INDEX IF NOT EXISTS idx_item_modifiers_item ON menu_item_modifiers(catalog_item_id);
CREATE INDEX IF NOT EXISTS idx_order_items_modifiers ON order_items USING GIN(selected_modifiers) WHERE selected_modifiers != '[]'::JSONB;
