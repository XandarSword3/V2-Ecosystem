...
CREATE TABLE IF NOT EXISTS guest_pass_usage (
    ...
    pool_ticket_id UUID REFERENCES transactions(id) ON DELETE SET NULL -- shared_capacity_access transaction
);
...
-- Pool capacity trigger fires on transactions (shared_capacity_access engine)
CREATE OR REPLACE FUNCTION update_pool_capacity()
RETURNS TRIGGER AS $$
BEGIN
    -- Only act on shared_capacity_access transactions
    IF NEW.engine_type != 'shared_capacity_access' THEN
        RETURN NEW;
    END IF;

    IF TG_OP = 'INSERT' AND NEW.status IN ('confirmed', 'pending') THEN
        INSERT INTO pool_daily_capacity (date, current_count)
        VALUES (((NEW.metadata->>'date')::date), COALESCE((NEW.metadata->>'quantity')::int, 1))
        ON CONFLICT (date)
        DO UPDATE SET
            current_count = pool_daily_capacity.current_count + COALESCE((NEW.metadata->>'quantity')::int, 1),
            updated_at = NOW();
        RETURN NEW;
    ELSIF TG_OP = 'UPDATE' AND NEW.status = 'cancelled' AND OLD.status != 'cancelled' THEN
        UPDATE pool_daily_capacity
        SET current_count = GREATEST(0, current_count - COALESCE((OLD.metadata->>'quantity')::int, 1)),
            updated_at = NOW()
        WHERE date = (OLD.metadata->>'date')::date;
        RETURN NEW;
    END IF;
    RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS pool_capacity_trigger ON transactions;
