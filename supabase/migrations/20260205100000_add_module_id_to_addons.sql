...
BEGIN
  ...
  ELSE
    RAISE NOTICE 'accommodation_add_ons table not found';
  END IF;
END $$;

-- Backfill existing add-ons to the first multi_day_booking module (if any)
DO $$
DECLARE
  v_module_id UUID;
BEGIN
  -- Find the first multi_day_booking module by creation date
  ...
  IF v_module_id IS NOT NULL THEN
    UPDATE accommodation_add_ons SET module_id = v_module_id WHERE module_id IS NULL;
    RAISE NOTICE 'Updated existing add-ons to module %', v_module_id;
  ELSE
